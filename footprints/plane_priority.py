...
from rubin_sim.scheduler.utils import (make_rolling_footprints)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gal_priority", type=float, default=0.6)
    parser.add_argument("--pencilbeams", dest='pencilbeams', action='store_true')
    parser.set_defaults(pencilbeams=False)
    ...
    dbroot = args.dbroot
    gsw = args.gsw
    gal_priority = args.gal_priority
    pencilbeams = args.pencilbeams
    ...
    if dbroot is None:
        ...
        fileroot = dbroot + '_'
    file_end = 'priority%.1f_' % gal_priority + '%s_' % tag + 'v2.1_'

    sm = gal_plane_footprint_generator(nside=nside)
    sm.set_params(priority_threshold=gal_priority, pencilbeams=pencilbeams)
